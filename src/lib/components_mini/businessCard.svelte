...
<div class="section-container">
    ...
    {#each businesses as {name, address, courses, business_id} (business_id)}
    <div class="business-container data-business_Id={business_id}">
        <div class="business-name"> Location Name: {name} </div>
        <div class="business-address"> address: {address} </div>
        <div class="business-business_id">Number of Courses: {courses.length} </div>
        <div class="business-business_id"> {business_id} </div>
      

      <div class="courses-container" style="display: none;">
      {#each courses as {course_id, name, holeCount, coursePar, holes} (course_id)}
      <div class="course-container data-course_id={course_id}">
        <div class="course-name">Course: {name} </div>
        <div class="course-coursePar">Course Par: {coursePar} </div>
        <div class="course-holeCount">Hole Count: {holeCount} </div>

          <div class="holes-container" style="display: none;">
          {#each holes as {hole_id, name, par} (hole_id)}
            <div class="hole-container data-hole_id={hole_id}"> 
              <div class="hole-name">Hole Name: {name} </div>
              <div class="hole-Par">Hole Par {par} </div>
              
              <div class="players-container" style="display: none;">
              {#each players as {player_id, playerImage ,name} (player_id)}
                <div class="player-container data-player_id={player_id}">
                  <img src={playerImage} class="player-img" alt="" width="50" height="50"> 
                  <div class="player-name">{name} </div>
                  <div class="score">
                    <form>
                      <select id={hole_id} name="score">
                      <option value=""></option>
